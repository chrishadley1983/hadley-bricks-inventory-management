/**
 * Migration script for importing purchases from Google Sheets to Supabase
 *
 * Usage:
 *   npx tsx scripts/migrate-purchases.ts [options]
 *
 * Options:
 ...
 *
 * Examples:
 *   npx tsx scripts/migrate-purchases.ts --dry-run --limit=10
 *   npx tsx scripts/migrate-purchases.ts --user-id=abc123
 */
...
async function main() {
  ...
  console.log('Hadley Bricks - Purchases Migration');
  console.log('='.repeat(60));
  console.log('');
  ...
  const result = await migrationService.migratePurchases({
    dryRun,
    limit,
    userId,
    updateExisting,
