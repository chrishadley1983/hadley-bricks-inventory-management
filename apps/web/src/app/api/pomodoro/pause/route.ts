...
export async function POST() {
  try {
    ...
    // Find active session (work or break)
    ...
    if (findError) {
      console.error('[POST /api/pomodoro/pause] Error:', findError);
      return NextResponse.json({ error: 'Failed to find session' }, { status: 500 });
    }

    if (!session) {
      return NextResponse.json({ error: 'No active session to pause' }, { status: 400 });
    }
    ...
    if (updateError) {
      console.error('[POST /api/pomodoro/pause] Error:', updateError);
    ...
    console.error('[POST /api/pomodoro/pause] Error:', error);
