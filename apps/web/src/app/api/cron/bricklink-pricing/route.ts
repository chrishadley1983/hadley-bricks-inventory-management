/**
 * POST /api/cron/bricklink-pricing
 *
 * Cron endpoint for daily BrickLink pricing sync using cursor-based batching.
 ...
 *
 * Recommended schedule: Daily at 2:30am UTC (GitHub Actions calls repeatedly until complete)
 * Staggered 30 minutes after eBay to avoid overlapping API load.
 */
...
const JOB_TYPE = 'bricklink_scheduled_pricing';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const bricklinkService = new BrickLinkArbitrageSyncService(supabase);
    ...
    console.log(`[Cron BrickLinkPricing] Starting sync - date: ${today}, cursor: ${cursorPosition}, isNewDay: ${isNewDay}`);
    ...
    if (processedToday >= DAILY_LIMIT) {
      console.log(`[Cron BrickLinkPricing] Daily limit reached (${DAILY_LIMIT} items)`);
    ...
    if (cursorPosition === 0) {
      console.log(`[Cron BrickLinkPricing] Starting new daily sync - ${totalWatchlist} items in watchlist, limit ${DAILY_LIMIT}`);
      await discordService.sendSyncStatus({
        title: '🔄 BrickLink Pricing Sync Started',
    ...
    if (batchLimit <= 0) {
      console.log(`[Cron BrickLinkPricing] No more items to process today`);
      return NextResponse.json({
        success: true,
        complete: true,
        processedToday,
    ...
    const result = await bricklinkService.syncPricingBatch(
      DEFAULT_USER_ID,
      {
    ...
    if (updateError) {
      console.error('[Cron BrickLinkPricing] Failed to update sync status:', updateError);
    }
    ...
    console.log(`[Cron BrickLinkPricing] Batch complete: ${result.processed} processed, cursor now at ${newCursorPosition}/${DAILY_LIMIT} (${durationStr})`);
    ...
    if (isComplete) {
      ...
      if (totalFailed > 0) {
        await discordService.sendSyncStatus({
          title: '⚠️ BrickLink Pricing Sync Complete (with errors)',
          message: `Updated: ${newCursorPosition} sets\nFailed: ${totalFailed} sets`,
          success: false,
        });
      } else {
        await discordService.sendSyncStatus({
          title: '✅ BrickLink Pricing Sync Complete',
          message: `Updated: ${newCursorPosition} sets`,
          success: true,
        });
      }
    ...
    await discordService.sendAlert({
      title: '🔴 BrickLink Pricing Sync Failed',
