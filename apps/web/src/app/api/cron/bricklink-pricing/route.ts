...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[Cron BrickLinkPricing] Starting sync - date: ${today}, cursor: ${cursorPosition}, isNewDay: ${isNewDay}`
    );
    ...
    if (cursorPosition === 0) {
      console.log(
        `[Cron BrickLinkPricing] Starting new daily sync - ${totalWatchlist} items in watchlist, limit ${DAILY_LIMIT}`
      );
    ...
    const result = await bricklinkService.syncPricingBatch(DEFAULT_USER_ID, {
    ...
    console.log(
      `[Cron BrickLinkPricing] Batch complete: ${result.processed} processed, cursor now at ${newCursorPosition}/${DAILY_LIMIT} (${durationStr})`
    );
