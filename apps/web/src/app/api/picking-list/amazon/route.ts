...
export async function GET(request: NextRequest) {
  try {
    ...
    const response: AmazonPickingListResponse = {
      ...
      generatedAt,
      pickUrl: `${process.env.NEXT_PUBLIC_APP_URL || 'https://hadley-bricks-inventory-management.vercel.app'}/pick/amazon`,
    };
    ...
    serviceClient
      ...
      .then(({ error: snapError }) => {
        if (snapError) {
          console.error('[GET /api/picking-list/amazon] Failed to save snapshot:', snapError);
          return;
        }
        ...
        serviceClient
          ...
          .then(({ error: cleanupError }) => {
            if (cleanupError) {
              console.error('[GET /api/picking-list/amazon] Failed to clean old snapshots:', cleanupError);
            }
          });
      });
