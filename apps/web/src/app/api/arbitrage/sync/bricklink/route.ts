/**
 * BrickLink Pricing Sync API Route with Streaming Progress
 *
 * POST /api/arbitrage/sync/bricklink - Trigger BrickLink pricing sync with progress streaming
 */

import { NextResponse } from 'next/server';
import { createClient } from '@/lib/supabase/server';
import { ArbitrageService, BrickLinkArbitrageSyncService } from '@/lib/arbitrage';
  ...
  try {
    ...
    const syncPromise = (async () => {
      ...
      try {
        ...
        const result = await brickLinkSyncService.syncPricing(
          user.id,
          { includeSeeded: true },
        ...
        console.error('[POST /api/arbitrage/sync/bricklink] Error:', error);
        await writer.write(
          encoder.encode(
            `data: ${JSON.stringify({
              type: 'error',
