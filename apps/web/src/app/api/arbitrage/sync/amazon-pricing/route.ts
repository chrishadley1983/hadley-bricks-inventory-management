/**
 * Amazon Pricing Sync API Route with Streaming Progress
 *
 * POST /api/arbitrage/sync/amazon-pricing - Trigger Amazon pricing sync with progress streaming
 */
  ...
  try {
    ...
    const syncPromise = (async () => {
      ...
      try {
        ...
        await writer.write(
          encoder.encode(`data: ${JSON.stringify({ type: 'start', message: 'Starting Amazon pricing sync...' })}\n\n`)
        );

        // Run sync with progress callback
        const result = await amazonSyncService.syncPricing(
          user.id,
          { includeSeeded: true },
        ...
        console.error('[POST /api/arbitrage/sync/amazon-pricing] Error:', error);
        await writer.write(
          encoder.encode(
            `data: ${JSON.stringify({
              type: 'error',
