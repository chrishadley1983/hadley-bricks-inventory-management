...
export async function POST(request: NextRequest): Promise<NextResponse<any>> {
  ...
  if (scanType === 'watchlist' && !setNumber) {
    return NextResponse.json(
      { error: 'setNumber required for watchlist scan' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const uniqueSetNumbers = [
      ...new Set(filteredListings.map((l) => l.extractedSetNumber)),
    ];
    ...
    const amazonPricing = new Map<string, { asin: string | null; ukRetailPrice: number | null; setName: string | null; buyBoxPrice?: number | null }>();
    ...
    if (scanType === 'watchlist' && setNumber) {
      ...
      try {
        ...
        if (credentials) {
          ...
          const pricesWithBuyBox = await asinService.getAmazonPrices(uniqueSetNumbers, amazonClient);
        ...
        if (existingStats && existingStats.total_scans > 1) {
          ...
          await supabase
            ...
            .update({
              ...
              listings_found:
                (existingStats.listings_found || 0) + processedListings.length,
              viable_found: (existingStats.viable_found || 0) + viableListings.length,
              near_miss_found:
                (existingStats.near_miss_found || 0) + nearMissListings.length,
              updated_at: new Date().toISOString(),
            })
            .eq('user_id', userId)
            .eq('set_number', setNumber);
