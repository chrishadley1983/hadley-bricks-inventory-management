 ...
 * Get platform performance comparison report
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await reportingService.getPlatformPerformanceReport(auth.userId, dateRange);

    return NextResponse.json({ data: report });
  } catch (error) {
    console.error('[GET /api/reports/platform-performance] Error:', error);
