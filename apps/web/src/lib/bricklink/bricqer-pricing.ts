 ...
 *
 * Mirrors the live Bricqer formula (updated by Chris 2026-07-07):
 *
 *   max(0.0699, get_price('sold', 'avg_price', condition, 'UK') * multiplier)
 *
 *   sell_thru_rate = sold total_quantity / stock total_quantity   (UK, same condition)
 *
 *   New:  STR >= 0.5  -> 1.10   else 0.85
 *   Used: STR >= 1.5  -> 1.80   (super-velocity bracket, new in v3)
 *         STR >= 1.0  -> 1.40
 *         STR >= 0.75 -> 1.25
 *         STR >= 0.5  -> 1.15
 *         STR >= 0.25 -> 0.93
...
/** Bricqer's minimum auto-price (the 7p store floor, live since ~2026-06-12). */
export const BRICQER_PRICE_FLOOR = 0.0699;
...
/**
 * Projected Bricqer list price: UK 6-month sold avg × multiplier, floored at 7p.
