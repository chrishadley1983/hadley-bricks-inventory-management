...
export function RepricingRow({ item }: RepricingRowProps) {
  ...
  const [manualCost, setManualCost] = useState<string>(item.inventoryCost?.toFixed(2) ?? '');
  ...
  const effectiveCost = useManualCost ? parseFloat(manualCost) || null : item.inventoryCost;
  ...
  const currentDiff = item.effectivePrice !== null ? effectivePrice - item.effectivePrice : null;
  ...
  return (
    <TableRow
      className={cn('hover:bg-muted/30', priceChanged && 'bg-blue-50/30 dark:bg-blue-950/10')}
    >
      {/* ASIN */}
      ...
      <TableCell className="text-right">
        {isEditing ? (
          ...
          <div className="flex items-center justify-end gap-1">
            <span className={cn('font-mono', priceChanged && 'text-blue-600 font-semibold')}>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center justify-end gap-1">
                ...
                {item.buyBoxIsYours && <span className="text-xs text-green-600">(yours)</span>}
                {item.priceSource === 'lowest' && (
                  <Badge
                    variant="outline"
                    className="h-4 px-1 text-[10px] text-purple-600 border-purple-300"
                  >
            ...
            <TooltipContent side="left">
              <div className="text-xs space-y-1">
                {item.buyBoxPrice !== null && <p>Buy Box: {formatCurrency(item.buyBoxPrice)}</p>}
      ...
      <TableCell className="text-right">
        <div className="flex items-center justify-end gap-1">
          <span className="font-mono text-muted-foreground">{formatCurrency(item.wasPrice)}</span>
          {getTrendIndicator()}
        </div>
      </TableCell>
