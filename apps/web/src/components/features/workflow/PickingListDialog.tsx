...
export function PickingListDialog({
  open,
  onOpenChange,
  platform,
}: PickingListDialogProps) {
          ...
          {data && data.totalOrders > 0 && (
            <>
                ...
                <div className="space-y-2">
                  {data.unmatchedItems.length > 0 && (
                    <Alert className="bg-amber-50 border-amber-200 dark:bg-amber-950/20 dark:border-amber-900">
                      ...
                      <AlertDescription className="text-amber-800 dark:text-amber-200">
                        <strong>
                          {data.unmatchedItems.length} unmatched item(s)
                        </strong>{' '}
                        ...
                        <ul className="mt-2 text-sm space-y-1">
                          {data.unmatchedItems.slice(0, 5).map((item, i) => (
                            <li key={i} className="truncate">
                              &bull; {item.setNo || item.asin || 'No SKU'}:{' '}
                              {item.itemName}
                            </li>
                          ))}
                  ...
                  {data.unknownLocationItems.length > 0 && (
                    <Alert className="bg-amber-50 border-amber-200 dark:bg-amber-950/20 dark:border-amber-900">
                      ...
                      <AlertDescription className="text-amber-800 dark:text-amber-200">
                        <strong>
                          {data.unknownLocationItems.length} item(s) missing
                          location
                        </strong>{' '}
              ...
              <div className="space-y-4">
                {Array.from(itemsByLocation.entries()).map(
                  ([location, items]) => (
                    <div
                      key={location}
