...
2. Live proof: npx tsx scripts/store-report.ts --slug=thebrickshack_ (no lens flag) - the CLI header must print "UK-grounded" (NOT "ESTIMATE lens"), and the coverage line must show world (world) at or near 0% (world benchmarks are NOT silently used; former world lots resolve to "none"/gaps). Run again WITH --pricing-lens=estimate and confirm the header flips to "ESTIMATE lens" and world% rises - proving grounded is the default, estimate the opt-in.
...
3. Every row: cappedQty == null OR cappedQty <= qty; cappedLotNet ? netPerUnit x (cappedQty ?? qty) within 2p; benchProvenance in {uk,world,none}; strQty null or >= 0.
