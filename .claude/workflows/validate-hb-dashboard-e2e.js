...
The dashboard is a single static page published to surge, focused ONLY on the website (hadleybricks.co.uk / Shopify) — NOT the marketplaces. The data is AES-encrypted into the page; a client-side PBKDF2+AES-GCM gate decrypts it in-browser on passcode submit (the passcode is NEVER in the HTML). Sections: a Position card with an AI-written summary + KPI strip, Search Ranking (Search Console), Google Shopping (Merchant feed + Shopping traffic), Shopify website & AI visibility (GA4), Store (listings). Generator/data: ${REPO}/apps/web/scripts/dashboard/ (build-dashboard.ts, data.ts, template.html). For a read-only live data pull, write a throwaway tsx in scripts/dashboard/ that imports { buildDashboardData } from './data' and prints the result, run with: cd ${REPO}/apps/web && npx tsx --env-file=.env.local scripts/dashboard/<your>.ts
...
const DIMENSIONS = [
  ...
  {
    key: 'unlock-render',
    prompt: `Drive the live page in a real headless browser (Playwright). (1) Load ${URL}; confirm the gate shows. (2) Enter the correct passcode "${PASS}", submit, and confirm the dashboard unlocks and #app becomes visible. (3) Confirm ALL sections render: Position card with a non-empty AI summary + KPI strip, Search Ranking, Google Shopping, Shopify website & AI visibility, Store. (4) Confirm the Chart.js charts actually draw (a dual-axis clicks-vs-position ranking chart + a daily-sessions chart) (canvas has non-blank pixels / a Chart instance) and are NOT absurdly tall (the known bug). (5) Capture console errors — there must be none that break rendering. Verdict FAIL if it doesn't unlock or a section/chart is missing/broken.`,
  },
  {
    key: 'data-integrity',
    prompt: `Confirm the dashboard shows REAL, FRESH data (not placeholders/stale). Unlock in a browser and read the rendered values, then independently pull the live data (write a throwaway tsx importing buildDashboardData from ./data, run with --env-file=.env.local) and cross-check: the Merchant feed approved/total, the Store listed count, and the Search Ranking organic clicks (28d) + avg position should match the dashboard (allowing for the dashboard being built slightly earlier). Confirm the "Updated …" timestamp / generatedAt is recent (within ~8 days). Verdict FAIL if values are obviously placeholder, zero across the board, or stale (>8 days), or contradict the live pull beyond normal drift.`,
  },
  {
    key: 'mobile',
