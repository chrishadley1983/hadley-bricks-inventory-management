-- Include seeded ASINs in arbitrage_current_view
-- Migration: 20260212000001_add_seeded_asins_to_arbitrage_view
--
-- Purpose: Extend arbitrage_current_view with UNION ALL to include seeded ASINs
-- that are not already in tracked_asins. This surfaces ~7,000+ seeded items
-- alongside the user's inventory items in the BrickLink and eBay arbitrage tabs.
--
-- Part A: Existing tracked_asins (item_type = 'inventory')
-- Part B: Seeded ASINs with pricing (item_type = 'seeded')
...
SELECT
  ...
  CASE
    WHEN COALESCE(ap.buy_box_price, ap.lowest_offer_price) > 0
         AND bp.min_price > 0
    THEN ROUND(
      (bp.min_price / COALESCE(ap.buy_box_price, ap.lowest_offer_price)) * 100,
      1
    )
    ELSE NULL
  END as cog_percent,
  ...
  -- eBay COG %
  CASE
    WHEN COALESCE(ap.buy_box_price, COALESCE(ap.your_price, sa.amazon_price)) > 0 AND ep.min_price > 0
    THEN ROUND(
      (ep.min_price / COALESCE(ap.buy_box_price, COALESCE(ap.your_price, sa.amazon_price))) * 100,
      1
    )
    ELSE NULL
  END as ebay_cog_percent,
  ...
  CASE
    WHEN COALESCE(ap.buy_box_price, COALESCE(ap.your_price, sa.amazon_price)) > 0 AND ep.min_price > 0
    THEN ROUND(((COALESCE(ap.buy_box_price, COALESCE(ap.your_price, sa.amazon_price)) - ep.min_price) / COALESCE(ap.buy_box_price, COALESCE(ap.your_price, sa.amazon_price))) * 100, 1)
    ELSE NULL
  END as ebay_margin_percent,

  CASE
    WHEN COALESCE(ap.buy_box_price, COALESCE(ap.your_price, sa.amazon_price)) > 0 AND ep.min_price > 0
    THEN ROUND(COALESCE(ap.buy_box_price, COALESCE(ap.your_price, sa.amazon_price)) - ep.min_price, 2)
    ELSE NULL
  END as ebay_margin_absolute,
...
COMMENT ON VIEW arbitrage_current_view IS 'Denormalized view combining tracked ASINs AND seeded ASINs with latest pricing. Includes item_type discriminator, COG% with BL override, and seeded-specific fields.';

-- ============================================================================
-- UPDATE RPC FUNCTIONS to include item_type and handle inventory/seeded filters
-- Must DROP first because return type is changing (added item_type column)
-- ============================================================================
...
COMMENT ON FUNCTION get_arbitrage_with_adjusted_ebay IS 'Fetches arbitrage data with exclusion-adjusted eBay prices for proper sorting. Supports inventory/seeded item type filters.';

-- Update count function to handle inventory/seeded filters
...
COMMENT ON FUNCTION get_arbitrage_adjusted_count IS 'Counts arbitrage items with exclusion-adjusted eBay filtering. Supports inventory/seeded item type filters.';
