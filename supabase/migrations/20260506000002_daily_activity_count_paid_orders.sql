-- Make the Amazon and eBay sales CTEs in daily_platform_activity consistent
-- with BrickLink/BrickOwl: count from "paid+", not just "shipped+/fulfilled+".
--
-- The previous Amazon filter required Shipped/Completed; the eBay filter
-- required order_fulfilment_status='FULFILLED'. Net effect: paid-but-not-yet-
-- dispatched orders never appeared in the daily report, so today's sales
-- pipeline was invisible to the view until each order had been packed and
-- the fulfilment status flipped.
--
-- New rules:
--  - Amazon: include internal_status IN ('Paid','Shipped','Completed') and the
--    raw-status fallbacks (so rows where internal_status is null but status
--    looks shipped/delivered still count). Cancelled/Refunded continue to be
--    excluded by the absence from the IN list and the cancel/refund status
--    pattern guards.
--  - eBay:  switch the source-of-truth from order_fulfilment_status='FULFILLED'
--    to order_payment_status='PAID'. PAID excludes FULLY_REFUNDED and
--    PARTIALLY_REFUNDED. Add a cancel_status guard so cancelled-after-paid
--    orders drop out (cancel_status->>'cancelState' = 'CANCELED').
...
COMMENT ON VIEW daily_platform_activity IS 'Daily aggregated listing and sales activity per platform. Listings from inventory_items (Amazon/eBay) and bricklink_uploads (BrickLink). Sales: paid+ for all platforms — Amazon counts Paid/Shipped/Completed via platform_orders; eBay counts payment_status=PAID minus cancellations from ebay_orders; BrickLink + Brick Owl share the bricklink column and count any not-cancelled platform_orders row.';
