-- Fix Daily Activity Reporting - Remove duplicate BrickLink counting
-- Migration: 20250113000006_fix_daily_activity_no_duplicate_bricklink
--
-- Issue: BrickLink orders were being double-counted because they exist in BOTH:
--   1. bricklink_transactions (from BrickLink API sync)
--   2. platform_orders where platform = 'bricklink' (from Bricqer sync)
--
-- Solution: Use ONLY platform_orders for BrickLink sold data since that's where
-- Bricqer syncs the orders. The bricklink_transactions table is for transaction
-- staging/reporting, not for the daily activity view.
...
-- This is the ONLY source for BrickLink - avoids double-counting with bricklink_transactions
bricklink_sold AS (
  SELECT
    user_id,
    order_date::DATE AS activity_date,
