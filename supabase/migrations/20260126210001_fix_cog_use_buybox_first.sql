-- Fix: Use buy_box_price first (not your_price) for COG% calculations
-- This ensures consistency between database sorting and UI display
-- Previously: COALESCE(your_price, buy_box_price) → inconsistent with UI
-- Now: COALESCE(buy_box_price, your_price) → matches UI behavior
...
BEGIN
  ...
  WITH base_data AS (
    SELECT
      ...
      -- Calculate adjusted COG% using BUY_BOX_PRICE first (matching UI logic)
      CASE
        WHEN adj.adjusted_total_listings > 0
          AND COALESCE(v.buy_box_price, v.your_price) > 0
          AND adj.adjusted_min_price > 0
        THEN ROUND((adj.adjusted_min_price / COALESCE(v.buy_box_price, v.your_price)) * 100, 1)
        WHEN adj.adjusted_total_listings IS NULL
          AND COALESCE(v.buy_box_price, v.your_price) > 0
          AND v.ebay_min_price > 0
        THEN ROUND((v.ebay_min_price / COALESCE(v.buy_box_price, v.your_price)) * 100, 1)
        ELSE NULL
...
COMMENT ON FUNCTION get_arbitrage_with_adjusted_ebay IS 'Fetches arbitrage data with exclusion-adjusted eBay prices. Uses buy_box_price first for COG% calculations.';
COMMENT ON FUNCTION get_arbitrage_adjusted_count IS 'Counts arbitrage items with exclusion-adjusted eBay filtering. Uses buy_box_price first for COG% calculations.';
