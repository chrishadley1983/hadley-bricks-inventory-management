-- Add profit_margin_percent to arbitrage_current_view
-- Accounts for Amazon FBM UK fees (18.36%) and tiered shipping (£3/<£14, £4/>=£14)
-- This enables server-side filtering on the true profit margin, not just simple (sell-buy)/sell
...
SELECT
    ...
    -- Amazon FBM profit margin: accounts for 18.36% fees + tiered shipping
    CASE
        WHEN COALESCE(ap.buy_box_price, ap.was_price_90d) > 0
             AND GREATEST(COALESCE(bp.min_price, 0::numeric), COALESCE(m.min_bl_price_override, 0::numeric)) > 0
        THEN round(
            (COALESCE(ap.buy_box_price, ap.was_price_90d)
             - COALESCE(ap.buy_box_price, ap.was_price_90d) * 0.1836
             - CASE WHEN COALESCE(ap.buy_box_price, ap.was_price_90d) < 14 THEN 3 ELSE 4 END
             - GREATEST(COALESCE(bp.min_price, 0::numeric), COALESCE(m.min_bl_price_override, 0::numeric))
            ) / COALESCE(ap.buy_box_price, ap.was_price_90d) * 100, 1)
        ELSE NULL
