-- Add price column to tracked_asins table
-- This stores the seller's listing price from the merchant listings report

ALTER TABLE tracked_asins
ADD COLUMN IF NOT EXISTS price DECIMAL(10,2);

COMMENT ON COLUMN tracked_asins.price IS 'Seller listing price from Amazon merchant listings report';

-- Update the view to use tracked_asins.price as fallback for your_price
...
COMMENT ON VIEW arbitrage_current_view IS 'Denormalized view combining tracked ASINs with latest pricing from Amazon, BrickLink, and eBay. Uses tracked_asins.price as fallback for your_price, and tracked_asins.quantity for your_qty.';
