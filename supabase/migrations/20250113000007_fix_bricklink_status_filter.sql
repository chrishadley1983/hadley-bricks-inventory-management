-- Fix Daily Activity Reporting - Relax BrickLink status filter
-- Migration: 20250113000007_fix_bricklink_status_filter
--
-- Issue: BrickLink orders from Bricqer have different status values than expected.
-- The filter was too restrictive and not matching any orders.
--
-- Solution: For BrickLink orders from platform_orders, include all orders that are
-- NOT pending/cancelled since Bricqer syncs completed orders.
...
-- Include ALL BrickLink orders except cancelled
-- Bricqer syncs completed orders so we trust them as sales
bricklink_sold AS (
  SELECT
    user_id,
    order_date::DATE AS activity_date,
  ...
  WHERE order_date IS NOT NULL
    ...
    AND (
      -- Exclude only if explicitly cancelled
      internal_status IS NULL OR internal_status != 'Cancelled'
    )
    AND (
      -- Also exclude if raw status indicates cancelled
...
-- Aggregate sold data by user/date/platform (handles BrickLink + BrickOwl combination)
sold_data AS (
  SELECT
    user_id,
    activity_date,
...
-- COMMENTS
-- ============================================================================
COMMENT ON VIEW daily_platform_activity IS 'Daily aggregated listing and sales activity per platform. Listings from inventory_items (Amazon/eBay) and bricklink_uploads (BrickLink). Sales from platform_orders (Amazon, BrickLink via Bricqer), ebay_orders (eBay), brickowl_transactions (BrickOwl combined with BrickLink).';
