-- Fix eBay condition case mismatch in arbitrage_current_view
-- The ebay_pricing table stores condition as 'NEW' (uppercase)
-- but the view was filtering for 'New' (capitalized)
...
COMMENT ON VIEW arbitrage_current_view IS 'Denormalized view combining tracked ASINs with latest pricing from Amazon, BrickLink, and eBay. Uses buy_box_price when available, falls back to lowest_offer_price. Fixed: eBay condition comparison is now case-insensitive.';
