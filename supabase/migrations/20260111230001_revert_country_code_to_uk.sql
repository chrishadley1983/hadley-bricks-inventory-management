-- Revert BrickLink country code back to UK
-- BrickLink uses non-standard "UK" instead of ISO "GB" for United Kingdom
-- Verified via API testing: countryCode=UK returns data, countryCode=GB returns 0 results

-- Revert existing data from 'GB' back to 'UK'
UPDATE bricklink_arbitrage_pricing
SET country_code = 'UK'
WHERE country_code = 'GB';

-- Revert the default value for the column
...
COMMENT ON VIEW arbitrage_current_view IS 'Denormalized view combining tracked ASINs with latest pricing from Amazon and BrickLink (UK sellers)';
