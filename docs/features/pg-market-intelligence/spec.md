...
| **C. `priceGuideSummary.asp` (anon curl)** | None — zero account risk | **Challenged after ~43 requests** at 4–6s jitter (2026-07-07 Jabbz fill: 35 ok / 8 fail, clean 3-consecutive abort) — but on a home IP that had already served ~1,400 catalogPG pages that day. Clean-IP ceiling untested; healthy rate ~8–10 req/min | **Elastic insurance**: build-phase gap fills, new-release first fetch, Event-tier top-ups during scans, overflow if lane D misses. Needs its own identity discipline (small sessions, breathers, never share an IP with same-day lane D). Steady-state usage decays to ~tens/day (§4.5) |
...
- **Nightly 00:00–07:00 (local bot, unattended):** lane D sessions → lane C event/residual queue → screens/report generation. Lane A live checks run on demand around the clock. **Lanes C and D must not share an exit IP on the same day** — the Jabbz fill showed challenge tolerance is (at least partly) per-IP, not per-mechanism, so "different account" isn't separation if both ride the home connection. Routing lane D through the VPN'd profile keeps the home IP clean for lane C.
...
1. `/define-done` for **P0** (§6 row 1 = its skeleton), branch `feature/pg-coverage-platform-p0`. Include the residual-fill service: resumable queue + curl→API lane rotation on challenge (the 2026-07-07 Jabbz fill aborted at 43/279 on a Cloudflare challenge — one-shot scripts don't survive contact; 244 tuples still open as its acceptance case).
