...
import { Loader2, Search, ExternalLink, TrendingUp, TrendingDown, AlertCircle } from 'lucide-react';
import {
  Table,
  TableBody,
  TableCell,
...
function getCogBadge(cogPercent: number | null) {
  ...
  if (cogPercent < 30) {
    return <Badge className="bg-green-600">🟢🟢 {cogPercent.toFixed(0)}%</Badge>;
  }
  if (cogPercent < 40) {
    return <Badge className="bg-green-500">🟢 {cogPercent.toFixed(0)}%</Badge>;
  }
  if (cogPercent < 50) {
    return <Badge className="bg-yellow-500">🟡 {cogPercent.toFixed(0)}%</Badge>;
  }
  if (cogPercent < 60) {
    return <Badge className="bg-orange-500">🟠 {cogPercent.toFixed(0)}%</Badge>;
  }
...
export default function VintedArbitragePage() {
  ...
  const { data, isLoading, error } = scanMutation;
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Scan Vinted listings and compare against Amazon Buy Box prices
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Slider
              value={[cogThreshold]}
              onValueChange={([v]) => setCogThreshold(v)}
              min={20}
              max={60}
              step={5}
              className="w-64"
      ...
      {data && (
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <Card className={data.summary.viableOpportunities > 0 ? 'border-green-500 bg-green-50' : ''}>
            <CardHeader className="pb-2">
                ...
                {data.viable.map((item) => (
                  <TableRow key={item.setNumber} className="bg-green-50/50">
                    ...
                    <TableCell className="text-right">
                      {formatCurrency(item.vintedPrice)}
                    </TableCell>
                    <TableCell className="text-right">
            ...
            <Table>
              ...
              <TableBody>
                {data.results.map((item) => (
                  <TableRow 
                    key={`${item.setNumber}-${item.vintedPrice}`}
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            <strong>Formula:</strong> COG% = (Vinted Price + £2.30 shipping) ÷ Amazon Price × 100
            <br />
