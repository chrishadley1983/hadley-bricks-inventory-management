...
The scanner uses a split architecture: **server generates schedules**, **local service executes scans**.
...
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  • Fetches current watchlist (200 sets)                      │   │
│  │  • Shuffles order (seeded by date)                           │   │
│  │  • Assigns random times (2-8 min gaps)                       │   │
│  │  • Adds broad sweep slots (1/hour, random 0-55 min)          │   │
│  │  • Ensures 5+ min gap between broad sweep and watchlist      │   │
│  │  • Returns sorted schedule for the day                       │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
...
└─────────────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│                 LOCAL SERVICE (Windows Native Tray App)             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────────┐  │
...
│              Next scan: 14:23 - LEGO 75192                          │
│              Today: 47/214 scans complete                           │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**Requirements:**
- Windows PC powered on during operating hours (08:00-22:00)
- Chrome browser installed with "Claude in Chrome" extension
- Claude CLI installed with valid API key
- Hadley Bricks Scanner tray app running (auto-starts with Windows)

### 2.4 Schedule Generation (Server-Side)

The server generates a complete daily schedule with randomised timing.

#### 2.4.1 Schedule API Response
...
#### 2.4.2 Schedule Generation Rules

| Rule | Implementation |
|------|----------------|
| Broad sweep frequency | Once per hour during operating hours |
| Broad sweep timing | Random 0-55 minutes into each hour |
| Watchlist coverage | All 200 sets distributed across operating hours |
| Watchlist gaps | Random 2-8 minutes between scans |
| Broad/watchlist separation | Minimum 5 minutes gap between broad sweep and nearest watchlist scan |
| Daily shuffle | Watchlist order randomised each day (seeded by date) |
| Reproducibility | Same date + same watchlist = same schedule (for debugging) |

#### 2.4.3 Schedule Generation Algorithm
...
function generateDailySchedule(
  ...
  const startHour = parseInt(config.operatingHoursStart);
  const endHour = parseInt(config.operatingHoursEnd);
  
  // 1. Generate broad sweep slots (one per hour, random minute)
  const broadSweepTimes: Date[] = [];
  for (let hour = startHour; hour < endHour; hour++) {
    const randomMinute = seededRandom(date, `broad_${hour}`, 0, 55);
    const time = new Date(date);
    time.setHours(hour, randomMinute, 0, 0);
    broadSweepTimes.push(time);
    
    scans.push({
      id: `broad_${date.toISOString().split('T')[0]}_${hour}`,
      scheduledTime: formatTime(time),
      type: 'broad_sweep'
    });
  }
  
  // 2. Shuffle watchlist (seeded by date for reproducibility)
  const shuffled = seededShuffle([...watchlist], date);
  
  // 3. Distribute watchlist scans with random gaps
  let currentTime = new Date(date);
  currentTime.setHours(startHour, 0, 0, 0);
  
  for (const item of shuffled) {
    // Random gap: 2-8 minutes
    const gapMinutes = seededRandom(date, item.setNumber, 2, 8);
    currentTime = new Date(currentTime.getTime() + gapMinutes * 60 * 1000);
    
    // Check for broad sweep collision (must be 5+ min apart)
    for (const bsTime of broadSweepTimes) {
      const diffMs = Math.abs(currentTime.getTime() - bsTime.getTime());
      if (diffMs < 5 * 60 * 1000) {
        // Push watchlist scan to 5 min after broad sweep
        currentTime = new Date(bsTime.getTime() + 5 * 60 * 1000);
      }
    }
    
    // Don't schedule past end of operating hours
    if (currentTime.getHours() >= endHour) break;
    
    scans.push({
      id: `watchlist_${date.toISOString().split('T')[0]}_${item.setNumber}`,
  ...
  // 4. Sort by time
  return scans.sort((a, b) => a.scheduledTime.localeCompare(b.scheduledTime));
}

// Seeded random for reproducibility
function seededRandom(date: Date, salt: string, min: number, max: number): number {
  const seed = `${date.toISOString().split('T')[0]}_${salt}`;
  const hash = cyrb53(seed);
  return min + (hash % (max - min + 1));
}
```

#### 2.4.4 Schedule Regeneration

When the watchlist is refreshed mid-day:

1. Server increments `scheduleVersion`
2. Server regenerates schedule for **remaining hours only**
3. Already-executed scans retained in history
4. Local service detects version change on next config poll
5. Local service fetches new schedule immediately
6. New/removed sets take effect for remaining day

### 2.5 Local Windows Service

#### 2.5.1 Technology

**Native Windows application** using:
- .NET 8 / C# for main application
...
1. Installs tray application to `%LOCALAPPDATA%\HadleyBricks\Scanner`
2. Creates Start Menu shortcut
3. Registers for auto-start with Windows (optional, user choice)
4. Validates Claude CLI is installed
5. Validates Chrome extension is present
6. Prompts for Hadley Bricks API key

#### 2.5.3 System Tray Interface

```
┌─────────────────────────────────────┐
│  Hadley Bricks Scanner              │
├─────────────────────────────────────┤
│  Status: ● Running                  │
│  Next: 14:23 - LEGO 75192           │
│  Today: 47/214 scans (2 opps)       │
├─────────────────────────────────────┤
│  ▶ Resume / ⏸ Pause                 │
│  🔄 Refresh Schedule                │
│  📊 Open Dashboard                  │
...
| 🟡 Yellow | Paused (manual or CAPTCHA) |
| 🔴 Red | Error (Chrome not found, API unreachable, etc.) |
| ⚪ Grey | Outside operating hours |

#### 2.5.4 Service Behaviour

**On startup:**
1. Check Chrome is installed
2. Check Claude CLI is installed and authenticated
3. Fetch config from server (`GET /config`)
4. Fetch today's schedule from server (`GET /schedule`)
5. Register system tray icon
6. Start scheduler engine

**Main loop (every 30 seconds):**
```
1. If outside operating hours → sleep
2. If config.paused or !config.enabled → sleep
3. Check if any scan is due (scheduledTime <= now AND not executed)
4. If scan due:
   a. Execute via Claude CLI + Chrome
   b. POST results to server
   c. Mark scan as executed locally
   d. Update tray tooltip
5. Send heartbeat to server (every 5 min)
```

**Config polling (every 5 minutes):**
1. Fetch config from server
2. If `paused` changed → update local state
3. If `scheduleVersion` changed → fetch new schedule

**At midnight (or on date change):**
1. Clear executed flags
2. Fetch new day's schedule

**Error handling:**
- Chrome not responding → retry 3x, then pause and alert
- API unreachable → use cached schedule, retry connection
- Claude CLI fails → log error, skip scan, continue
- CAPTCHA detected → pause immediately, notify user

#### 2.5.5 Missed Scan Behaviour

If the PC was off or service wasn't running:

| Scenario | Behaviour |
|----------|-----------|
| Missed scans | **Skip entirely** - do not attempt catch-up |
| Resume mid-day | Continue from next scheduled scan |
| Large gap (>2 hours) | Log warning, continue normally |

**Rationale:** Catch-up would create burst of activity that looks suspicious. Better to skip and resume normal cadence.

#### 2.5.6 Logs

Local logs stored at `%LOCALAPPDATA%\HadleyBricks\Scanner\logs\`:
- `scanner-YYYY-MM-DD.log` - Daily log file
- Retention: 30 days

Log entries include:
- Scan executions (start, end, result)
- API communications
- Errors and warnings
- Schedule fetches
...
Local service sends heartbeat every 5 minutes:

```typescript
// POST /api/arbitrage/vinted/automation/heartbeat

interface HeartbeatRequest {
  machineId: string;           // Unique ID for this installation
  status: 'running' | 'paused' | 'error';
...
interface HeartbeatResponse {
  configVersion: number;       // If changed, refetch config
  scheduleVersion: number;     // If changed, refetch schedule
  serverTime: string;          // For clock sync check
}
```

#### 2.6.2 Dashboard Integration

The web dashboard shows local scanner status:

```
┌─────────────────────────────────────────────────────────────────────┐
│  Scanner Status                                                      │
├─────────────────────────────────────────────────────────────────────┤
│  Local Service: ● Connected (last seen 2 min ago)                   │
│  Status: Running                                                     │
│  Machine: CHRIS-PC                                                   │
│  Today: 47/214 scans complete, 2 opportunities found                │
│  Next scan: 14:23 - LEGO 75192 Millennium Falcon                    │
└─────────────────────────────────────────────────────────────────────┘
```
...
│  Scanner Status                                                      │
├─────────────────────────────────────────────────────────────────────┤
│  Local Service: ⚠️ Not Connected                                    │
│  Last seen: 47 minutes ago                                          │
│                                                                      │
│  The scanner runs on your Windows PC. Please ensure:                │
│  • Your PC is powered on                                            │
│  • Hadley Bricks Scanner is running (check system tray)             │
│  • You have internet connectivity                                   │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.7 Data Extraction Method
